      ...
      <div class="cart-body">
        ...
        >
          ...
          <li class="cart-list-con5">
            <a class="mins" @click="handler(-1, -1, cart)">-</a>
            <input
              autocomplete="off"
              type="text"
              :value="cart.skuNum"
              ...
              ref="inputs"
            />
            <a class="plus" @click="handler(1, 1, cart)">+</a>
          </li>
          <li class="cart-list-con6">
            <span class="sum">{{ cart.skuNum * cart.skuPrice }}</span>
          </li>
          <li class="cart-list-con7">
            <a href="#none" class="sindelet" @click="deleteShopCartById(cart)"
              >删除</a
            >
            <br />
            <a href="#none">移到收藏</a>
...
export default {
  ...
  methods: {
    ...
    // 修改想要买的产品个数,type参数是选择哪个元素，cart身上有产品id和身上的数量
    handler: throttle(async function (type, disNum, cart) {
      ...
      try {
        await this.$store.dispatch("detail/addOrUpdateShop", {
          skuId: cart.skuId,
          skuNum: disNum,
        });
        // 成功了就重新加载  必须有这个trycatch
        this.getData();
      } catch (error) {}
    }, 1000),
    // 删除按钮
    async deleteShopCartById(cart) {
      try {
        await this.$store.dispatch("shopCart/deleteShopCartById", cart.skuId);
        this.getData();
      } catch (error) {
